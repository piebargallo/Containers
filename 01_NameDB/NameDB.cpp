...
NameDB::NameDB(string_view nameFile)
{
	...
	while (inputFile >> name) {
		...
		if (nameExists(name)) {
			// If the name exits in the database, just increment the count.
			incrementNameCount(name);
		}
		else {
			// If the name doesn´t yet exist, add it witha count of 1.
			addNewName(name);
		}
	}
}
...
// Precondition: name exists in the vector of name.
...
// then iterates througth all the names, counting all the names with a higher.
// count then the specified name. Returns that count as the rank.

int NameDB::getNameRank(string_view name) const
{
	// Make use of the getAbsolutNumber() method.
	...
	// decreasses the rank of this name by 1.
	int rank{ 1 };
	for (auto& entry : m_names) {
		if (entry.second > num) {
			++rank;
